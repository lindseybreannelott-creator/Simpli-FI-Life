// --- HOME PAGE: FINAL GEOMETRIC RECALIBRATION ---
...
const TestimonialScroller = () => {
    ...
    // --- RESPONSIVE GEOMETRY ---
    // Desktop: 5 visible (20vw per card space)
    // Mobile: 1 centered, 2 peeking (80vw per card space)
    ...
    const CARD_WIDTH_VW = isMobile ? 75 : 18; 
    const GAP_VW = isMobile ? 5 : 2;
    ...
    const handleScroll = () => {
        ...
        // Math to ensure the 3rd card highlights exactly in the middle
        const index = Math.round(centerLine / cardFullWidth) % originalItems.length;
        setActiveIndex(index);

        // INFINITE LOOP RESET
        const setWidth = originalItems.length * cardFullWidth;
    ...
    useEffect(() => {
        if (containerRef.current) {
            const container = containerRef.current;
            const cardFullWidth = (container.offsetWidth * (CARD_WIDTH_VW / 100)) + (container.offsetWidth * (GAP_VW / 100));
    ...
    return (
        <div className="relative w-full py-12 group z-30">
            <div 
                ...
                style={{ 
                    paddingLeft: isMobile ? '7.5vw' : '2vw', 
                    paddingRight: isMobile ? '7.5vw' : '2vw' 
                }}
            >
        ...
        <div className="relative z-10 pt-40">
            ...
            <div className="relative">
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40">
                     <a href={link} target="_blank" rel="noopener noreferrer" className="block relative group bg-brand-periwinkle-light text-brand-dark px-12 py-4 font-display font-bold tracking-widest text-xs md:text-sm uppercase rounded-full shadow-xl hover:bg-white hover:scale-105 transition-all duration-300">
