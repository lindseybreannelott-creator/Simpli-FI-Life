// --- HOME PAGE: SPOTLIGHT & MOBILE CALIBRATION ---
...
const TestimonialScroller = () => {
    ...
    // Desktop: 320px cards. Mobile: 70% width for better "peek"
    const ITEM_WIDTH = isMobile ? window.innerWidth * 0.70 : 320;
    const GAP = isMobile ? 16 : 24;
    const TOTAL_SPACE = ITEM_WIDTH + GAP;
    const SIDE_PADDING = isMobile ? (window.innerWidth * 0.15) : (window.innerWidth / 2 - ITEM_WIDTH / 2);
    ...
    const handleScroll = () => {
        ...
        // --- THE IMAGINARY SPOTLIGHT ---
        // Find the absolute center of the visible container
        const viewportCenter = container.offsetWidth / 2;
        // Map that center to the scrolling content
        const spotlightPoint = scrollPos + viewportCenter;
        
        // Calculate which card is currently overlapping the spotlight
        // We subtract the SIDE_PADDING to align index 0 with scrollPos 0
...
const SocialSection = ({ platform, handle, link, children }) => (
    <div className="relative w-full bg-brand-base">
        ...
        <div className="relative z-10 pt-40 pb-0">
            <div className="text-center mb-12">
                <p className="font-sans text-xs font-bold tracking-[0.4em] text-brand-medium uppercase mb-3">Follow me on</p>
                <h3 className="font-display text-5xl md:text-7xl font-bold text-brand-dark uppercase tracking-tighter">{platform}</h3>
            </div>
            <div className="relative">
                <div className="w-full h-1.5 bg-brand-periwinkle-light relative z-20"></div>
                {children}
                {/* BUTTON POSITIONED HALF-AND-HALF OVER STRIP */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40">
                     <a href={link} target="_blank" rel="noopener noreferrer" className="block relative group bg-brand-periwinkle-light text-brand-dark px-12 py-5 font-display font-bold tracking-widest text-xs md:text-sm uppercase rounded-full shadow-2xl hover:bg-white hover:scale-105 transition-all duration-300">
