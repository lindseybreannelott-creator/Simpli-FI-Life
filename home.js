...
const TestimonialScroller = () => {
    const originalItems = typeof TESTIMONIALS !== 'undefined' && TESTIMONIALS.length > 0 ? TESTIMONIALS : [];
    
    // Guard: Don't render if no testimonials
    ...
    useEffect(() => {
        if (containerRef.current && originalItems.length > 0) {
            ...
            const itemWidth = isMobile ? container.offsetWidth * 0.65 : ITEM_WIDTH_DESKTOP;
            const gap = isMobile ? 12 : GAP_DESKTOP;
            container.scrollLeft = originalItems.length * (itemWidth + gap) * 2; 
            setTimeout(handleScroll, 100);
        }
    }, [isMobile, originalItems.length]);
        ...
        <div className="overflow-x-hidden bg-brand-base">
            ...
            <div className="relative overflow-hidden min-h-screen flex flex-col justify-start">
                <div className="absolute inset-0 z-0">
                    <div className="absolute inset-0 opacity-60 bg-[linear-gradient(to_right,#d4d7ff_1px,transparent_1px),linear-gradient(to_bottom,#d4d7ff_1px,transparent_1px)] bg-[size:40px_40px]"></div>
                    <GridBeams />
                </div>
                ...
                <div className="max-w-7xl mx-auto px-4 relative z-10 text-center">
                    ...
                    <div className="grid md:grid-cols-2 gap-8 md:gap-16 max-w-5xl mx-auto">
                        ...
                        <div className="group relative bg-brand-white rounded-3xl p-8 md:p-10 border-2 border-stone-100 flex flex-col items-center transition-all duration-300 hover:shadow-2xl hover:scale-[1.03]">
                            ...
                            <Link to="/residential-spaces" className="w-full bg-brand-periwinkle-light text-brand-dark px-6 py-4 rounded-2xl font-bold hover:bg-brand-periwinkle hover:text-brand-white transition-all uppercase tracking-widest text-[10px] md:text-xs shadow-md">Get Organized, Lose the Stress</Link>
                        </div>
                    </div>
                </div>
            </section>
